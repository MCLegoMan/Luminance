...
void main() {
    ...
    color = mix(color, mix(color, wither, _smoothstep(0.0, 0.5, distance(texCoord, vec2(0.5, 0.5)))), min(withered, 1.0));
    color = mix(color, mix(color, poison, _smoothstep(0.0, 0.5, distance(texCoord, vec2(0.5, 0.5)))), min(poisoned, 1.0));
    color = mix(mix(color, overlay, _smoothstep(0.0, 0.5, distance(texCoord, vec2(0.5, 0.5)))), color, min(health, 1.0));
    color = mix(mix(color, overlay, _smoothstep(0.0, 0.5, distance(texCoord, vec2(0.5, 0.5)))), color, min(hurt, 1.0));
